...
// Every image uploaded to the site that we want to show in the portfolio
// gallery, keyed by the manufacturing technology that produced it. This list
// used to live in the old src/pages/OurWork.tsx; moved here so it can be
// embedded below the DB-driven case-studies grid on the new Our Work page.
const ALL_IMAGES: { src: string; category: string }[] = [
...
const PortfolioGallery: React.FC<PortfolioGalleryProps> = ({
  ...
  intro = 'Real parts from recent runs, grouped by manufacturing technology. Click any image to zoom in.',
}) => {
  ...
  const categories = useMemo(
    () => ['All', ...Array.from(new Set(ALL_IMAGES.map((i) => i.category)))],
    [],
  );
  ...
  const openAt = (index: number) => {
    const clicked = visibleImages[index];
    setCurrentIndex(ALL_IMAGES.indexOf(clicked));
    setIsOpen(true);
  };

  const showPrev = () =>
    setCurrentIndex((idx) => (idx - 1 + ALL_IMAGES.length) % ALL_IMAGES.length);
  const showNext = () =>
    setCurrentIndex((idx) => (idx + 1) % ALL_IMAGES.length);
    ...
    <section className="section bg-brand-light">
      <div className="container-custom">
        <div className="max-w-3xl mx-auto text-center mb-10">
          ...
          <p className="text-brand-dark/70 leading-relaxed text-lg">{intro}</p>
        </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                selectedCategory === cat
                  ...
                  : 'bg-white text-brand-dark hover:bg-gray-200 border border-gray-200'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {visibleImages.map((img, idx) => (
            <button
              ...
              type="button"
              onClick={() => openAt(idx)}
              className="group relative aspect-[4/3] w-full overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm hover:shadow-md transition-all duration-300"
              aria-label={(t('our_work_open_image', 'Open image') as string) || 'Open image'}
            >
              <img
                src={img.src}
                alt={
                  (t('our_work_image_alt', 'Manufacturing portfolio image') as string) ||
                  'Manufacturing portfolio image'
                }
                loading="lazy"
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-5xl w-[96vw] p-0 bg-black/95 border-0">
          <VisuallyHidden>
            <DialogTitle>{t('our_work_image_alt', 'Portfolio image')}</DialogTitle>
          </VisuallyHidden>
          <div className="relative w-full h-[90vh] flex flex-col items-center justify-center">
            <button
              ...
              aria-label={(t('close', 'Close') as string) || 'Close'}
            >
              <X size={24} />
            </button>
            ...
            <button
              ...
              aria-label={(t('previous', 'Previous') as string) || 'Previous'}
            >
              <ChevronLeft size={32} />
            </button>
            ...
            <button
              ...
              aria-label={(t('next', 'Next') as string) || 'Next'}
            >
            ...
            <div className="relative max-w-full max-h-full p-4 flex items-center justify-center">
              <img
                src={ALL_IMAGES[currentIndex].src}
                alt={(t('our_work_image_alt', 'Portfolio image') as string) || 'Portfolio image'}
              ...
              <div className="absolute bottom-8 left-0 right-0 text-center">
                <span className="bg-black/60 text-white px-4 py-2 rounded-full backdrop-blur-md text-sm font-medium">
                  {ALL_IMAGES[currentIndex].category}
                </span>
              </div>
            </div>
          </div>
