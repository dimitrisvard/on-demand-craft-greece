...
export async function fetchSpain() {
  ...
  // Strategy 4: TED search for Spanish tenders as additional source
  try {
    await delay(500);
    const tedUrl = 'https://ted.europa.eu/en/search/result?SF_S_CPV%5B%5D=42000000&SF_S_COUNTRY%5B%5D=ES';
    ...
    errors.push(`TED ES search: ${err.message}`);
  }
...
function parseTedHtmlFallback(html) {
  ...
  while ((m = linkRx.exec(html)) !== null && tenders.length < 30) {
    ...
    const ref = idM ? `ted-${idM[1]}` : `ted-es-${tenders.length}`;
    tenders.push({
