...
export async function fetchGermany() {
  ...
  // Strategy 4: Vergabe.NRW / regional portals
  try {
    await delay(600);
    const nrwUrl = 'https://www.vergabe.nrw.de/VMPSatellite/notice?cpv=42&status=ACTIVE&size=30';
    const resp = await fetchWithTimeout(nrwUrl, {
      headers: getBrowserHeaders('de-DE,de;q=0.9'),
    }, 20000);
    ...
    errors.push(`Vergabe.NRW: ${err.message}`);
  }

  // Strategy 5: TED search for German tenders as additional source
  try {
    await delay(500);
    const tedUrl = 'https://ted.europa.eu/en/search/result?SF_S_CPV%5B%5D=42000000&SF_S_COUNTRY%5B%5D=DE';
    ...
    errors.push(`TED DE search: ${err.message}`);
  }
...
function parseTedHtmlFallback(html) {
  ...
  while ((m = linkRx.exec(html)) !== null && tenders.length < 30) {
    ...
    const ref = idM ? `ted-${idM[1]}` : `ted-de-${tenders.length}`;
    tenders.push({
      countryCode: COUNTRY_CODE,
      countryName: COUNTRY_NAME,
