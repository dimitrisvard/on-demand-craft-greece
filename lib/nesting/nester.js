...
function placeParts(preparedParts, effectiveW, effectiveH, edgeMargin, sheetW, sheetH, gap) {
  ...
  for (const part of preparedParts) {
    ...
    for (let si = 0; si < sheets.length; si++) {
      ...
      if (result) {
        const score = result.x * 1e9 + result.y; // lower is better — pack left-to-right first
...
function findBestPosition(part, sheet, effectiveW, effectiveH, gap) {
  ...
  for (let ri = 0; ri < part.rotations.length; ri++) {
    ...
    for (const cp of sheet.candidatePositions) {
      ...
      const score = cx * 1e9 + cy; // pack left-to-right first
...
export async function nestGroup(parts, config) {
  ...
  for (const part of parts) {
    ...
    // Prefer the net area (outer minus holes) supplied by the DXF parser.
    // Fall back to outer polygon area only when no net area was supplied.
    const netArea = typeof part.area === 'number' && part.area > 0
      ? part.area
      : Math.abs(polygonArea(outerContour));
  ...
  // Primary sort: group by partId (alphabetical) so identical parts are adjacent,
  // then within each group sort by area descending for efficient packing.
  // This produces an ordered, systematic layout along the axis.
  const orderedParts = [...preparedParts].sort((a, b) => {
    // First group by partId for visual ordering
    if (a.partId !== b.partId) return a.partId.localeCompare(b.partId);
    // Within same part, sort by instance index
    return a.instanceIndex - b.instanceIndex;
  });

  // Also try area-descending for utilization comparison
  ...
  // Run ordered placement first (preferred — clean, systematic layout)
  let bestSheets = placeParts(orderedParts, effectiveW, effectiveH, edgeMargin, sheetW, sheetH, gap);
  let bestScore = scoreSolution(bestSheets, sheetW, sheetH);

  // Try area-sorted as alternative
  const areaSheets = placeParts(areaSorted, effectiveW, effectiveH, edgeMargin, sheetW, sheetH, gap);
  const areaScore = scoreSolution(areaSheets, sheetW, sheetH);
  if (isBetterSolution(areaScore, bestScore)) {
    bestSheets = areaSheets;
    bestScore = areaScore;
  }

  // Try a few random permutations for optimization
  ...
  const resultSheets = bestSheets.map((sheet) => {
    ...
    // Compute actual bounding box of all placed parts (used area on the sheet)
    let usedMinX = Infinity, usedMinY = Infinity, usedMaxX = -Infinity, usedMaxY = -Infinity;
    for (const pl of placements) {
      const left = pl.position.x;
      const bottom = pl.position.y;
      const right = left + pl.bbox.w;
      const top = bottom + pl.bbox.h;
      if (left < usedMinX) usedMinX = left;
      if (bottom < usedMinY) usedMinY = bottom;
      if (right > usedMaxX) usedMaxX = right;
      if (top > usedMaxY) usedMaxY = top;
    }
    const usedW = placements.length > 0 ? Math.round((usedMaxX - usedMinX) * 10) / 10 : 0;
    const usedH = placements.length > 0 ? Math.round((usedMaxY - usedMinY) * 10) / 10 : 0;

    return {
      placements,
      utilization,
      partArea: Math.round(partArea * 100) / 100,
      usedBbox: { w: usedW, h: usedH },
    };
  });
