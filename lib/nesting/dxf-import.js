...
export function parseDXF(dxfContent, partMeta = {}) {
  ...
  for (let i = 0; i < cleanedContours.length; i++) {
    ...
    // Check multiple points to reliably detect holes: centroid + first point + midpoint
    const c = centroid(contour);
    const mid = contour[Math.floor(contour.length / 2)];
    const insideCount =
      (pointInPolygon(c, outer) ? 1 : 0) +
      (pointInPolygon(contour[0], outer) ? 1 : 0) +
      (pointInPolygon(mid, outer) ? 1 : 0);
    // Majority vote: if at least 2 of 3 test points are inside, treat as hole
    if (insideCount >= 2) {
  ...
  // Capture the translation applied by normalizeToOrigin so we can replay
  // it on bend lines — they must live in the same coordinate frame as the
  // normalized outer contour.
  const preBBox = boundingBox(outer);
  const dx = -preBBox.minX;
  const dy = -preBBox.minY;
  ...
  const normalizedBendLines = bendLines
    .map((bl) => ({
      start: { x: bl.start.x + dx, y: bl.start.y + dy },
      end: { x: bl.end.x + dx, y: bl.end.y + dy },
      direction: bl.direction,
    }))
    // Drop bend lines that fall entirely outside the outer bbox — they're
    // usually 3mm-overshoot extensions the DXF exporter draws past the part.
    .map((bl) => clipBendLineToBBox(bl, boundingBox(outer)))
    .filter(Boolean);
