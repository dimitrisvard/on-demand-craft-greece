...
export function generateSvgPreview(sheet, sheetSize, partsLookup) {
  ...
  for (const placement of sheet.placements) {
    ...
    // Multi-piece rendering: when the parser produced more than one
    // top-level outline (e.g. the unfolded sheet has disconnected
    // components), render every piece as its own polygon — anchored on the
    // SAME nesting outer so they preserve their relative offsets.
    const pieces =
    ...
    for (const piece of pieces) {
      // Piece outers are placed by re-using the nesting outer's rotation +
      // translation so that gaps between pieces survive the placement.
      const pieceOuterTransformed = mirrorY(
        transformHoleContour(piece.outer, part.outer, placement),
      );
      geomParts.push(
        `<polygon points="${svgPointsAttr(pieceOuterTransformed)}" ` +
          `fill="${hexToRgba(color, 0.55)}" stroke="${color}" stroke-width="2" />`,
      );

      if (piece.holes && piece.holes.length > 0) {
        for (const hole of piece.holes) {
          const holeTransformed = mirrorY(
            transformHoleContour(hole, part.outer, placement),
          );
          holeParts.push(
