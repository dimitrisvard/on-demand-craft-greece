...
export function generateSvgPreview(sheet, sheetSize, partsLookup) {
  ...
  // Geometry group: render in SVG coordinate space directly.
  // The nester uses CAD coordinates (Y-up), so we mirror Y to SVG (Y-down)
  // by transforming each point rather than using a group-level flip.
  // This keeps geometry and labels in the same coordinate space.
  const geomParts = [];
  const holeParts = [];
  ...
  for (const placement of sheet.placements) {
    ...
    if (part.holes && part.holes.length > 0) {
      for (const hole of part.holes) {
        const holeTransformed = mirrorY(transformContour(hole, placement));
        holeParts.push(
  ...
  // Labels for parts (same coordinate space as geometry now)
  const labelParts = [];
  for (const placement of sheet.placements) {
    const part = partsLookup.find(
      (p) => p.partId === placement.partId || p.name === placement.partId,
    ...
    // White background rect behind label for readability
  ...
  <!-- Geometry group (mirrored per-point, no group transform needed) -->
  <g>
    ${geomParts.join('\n    ')}
    ${holeParts.join('\n    ')}
  </g>

  <!-- Part labels (same coordinate space as geometry) -->
  <g>
    ${labelParts.join('\n    ')}
  </g>
