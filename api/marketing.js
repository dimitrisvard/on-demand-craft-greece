...
//   action=apollo-enrich — Apollo.io contact enrichment (POST)
...
export default async function handler(req, res) {
  ...
  switch (action) {
    ...
    case 'apollo-enrich':
      return handleApolloEnrich(req, res);
    default:
      return res.status(400).json({ error: 'Invalid action. Use: track, webhook, google-auth, or apollo-enrich' });
  }
}
...
function apolloDelay(ms) {
  return new Promise(r => setTimeout(r, ms));
}

async function handleApolloEnrich(req, res) {
  ...
  if (!titles.length) return res.status(400).json({ error: 'titles array is required (job titles to search for)' });

  // Get API key from app_settings
  const { data: keyRow } = await supabase.from('app_settings').select('value').eq('key', 'apollo_api_key').single();
  const apiKey = keyRow?.value || process.env.APOLLO_API_KEY || '';
  if (!apiKey) return res.status(400).json({ error: 'Apollo API key not configured. Go to Settings > Integrations to add it.' });
  ...
  for (let i = 0; i < companies.length; i += 3) {
    ...
    if (i + 3 < companies.length) await apolloDelay(1500);
  }
