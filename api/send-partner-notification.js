...
export default async function handler(req, res) {
  ...
  try {
    console.log('[API] Partner notification handler start');
    ...
    if (!partnerEmail || !partnerName || !orderId || !orderTitle || !startDate || !deliveryDate) {
      return res.status(400).json({ 
        error: 'Missing required fields: partnerEmail, partnerName, orderId, orderTitle, startDate, deliveryDate are required' 
      });
    }

    console.log('Sending partner notification email for order:', orderId);
    ...
    const generateOrderItemsHTML = (items) => {
      if (!items || items.length === 0) {
        return '<p style="color: #64748b; font-style: italic;">No items specified</p>';
      }
    ...
    const partnerEmailResult = await resend.emails.send({
      ...
      to: [partnerEmail],
      subject: `New Production Order Received - ${orderId}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f0f9ff; border: 2px solid #0ea5e9; padding: 25px; border-radius: 12px; margin: 20px 0;">
            ...
            <p style="color: #0c4a6e; font-size: 16px; line-height: 1.6; margin: 0 0 20px 0;">
              We are pleased to assign a new production order to your company. Please review the details below and confirm your acceptance.
            </p>
          </div>
          ...
          <div style="background: #f8fafc; padding: 25px; border-radius: 12px; margin: 20px 0;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
              <div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
                ...
                <p style="color: #1f2937; font-size: 18px; margin: 0; font-family: monospace;">${orderId}</p>
              </div>
              <div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
            ...
            <div style="background: #f3f4f6; padding: 15px; border-radius: 8px; text-align: center; margin-bottom: 20px;">
              ...
              <p style="color: #1f2937; font-size: 20px; margin: 0; font-weight: bold;">${currency || 'EUR'} ${totalAmount.toLocaleString()}</p>
            </div>
            ` : ''}
          </div>
    ...
    console.log('Partner notification email sent:', partnerEmailResult);
    ...
    return res.status(500).json({ 
      error: 'Failed to send partner notification email', 
