import { SERVICE_IDS, ServiceId } from './types';

// Edge-safe service data. Mirrors src/pages/services/data.ts but without the
// lucide-react icon imports (which would pull React into the edge bundle).
//
// NOTE: in src/pages/services/data.ts the surface-finishes service is keyed as
// 'surface-finish' (singular) internally while its URL slug is 'surface-finishes'.
// Here and everywhere else in the middleware the plural URL-slug form is the
// canonical id.

export interface EdgeService {
  id: ServiceId;
  image: string;
  translationKey: string; // prefix used in service_<key>_* translation entries
  seoKeyBase: string;    // prefix used in seo_<base>_* translation entries
  capabilities: string[];
}

export const SERVICES: Record<ServiceId, EdgeService> = {
  'cnc-machining': {
    id: 'cnc-machining',
  ...
  'sheet-metal': {
    ...
    translationKey: 'sheet-metal',
    seoKeyBase: 'sheet_metal',
    capabilities: [
  ...
  'injection-molding': {
    ...
    translationKey: 'injection-molding',
    seoKeyBase: 'injection_molding',
    capabilities: [
  ...
  'surface-finishes': {
    ...
    translationKey: 'surface-finish',
    seoKeyBase: 'surface_finishes',
    capabilities: [
  ...
  'rapid-prototyping': {
    ...
    translationKey: 'rapid-prototyping',
    seoKeyBase: 'rapid_prototyping',
    capabilities: [
...
export const INDUSTRIES: { id: string; translationKey: string }[] = [
  { id: 'aerospace', translationKey: 'aerospace' },
  { id: 'automotive', translationKey: 'automotive' },
  { id: 'medical', translationKey: 'medical' },
  { id: 'electronics', translationKey: 'electronics' },
  { id: 'consumer-goods', translationKey: 'consumer-goods' },
  { id: 'industrial-equipment', translationKey: 'industrial-equipment' },
  { id: 'energy', translationKey: 'energy' },
  { id: 'robotics', translationKey: 'robotics' },
];
