...
3. **Sitemap Update**: 11:00 AM Greece / 9:00 AM UTC (1 hour after translation)
   - Cron job: `auto-update-sitemap`
   - Schedule: `0 9 * * *`
   - Function: `auto-update-sitemap`
   - Updates sitemap after translations are complete
...
11:00 AM Greece (9:00 AM UTC)   → Sitemap Update (1 hour later)
```
...
SELECT 
  ...
  END as description
FROM cron.job 
WHERE jobname IN ('enqueue-daily-article', 'auto-translate-daily-articles', 'auto-update-sitemap')
ORDER BY schedule;
```
